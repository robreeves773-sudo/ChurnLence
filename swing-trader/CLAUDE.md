...
6. `user_data/config-private.json` holds this machine's dashboard secrets
   (generated by `scripts/make_private_config.py`). It is gitignored — NEVER
   commit it, print its contents into logs/reports, or copy its values into
   any tracked file. To rotate secrets run the script with `--rotate`.
